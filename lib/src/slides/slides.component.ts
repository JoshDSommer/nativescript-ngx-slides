...
export class SlidesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @Output("tap")
  tap: EventEmitter<gestures.GestureEventData> = new EventEmitter<
    gestures.GestureEventData
  >();
  ...
  private _init() {
    if (
      ...
      (!this.manualInitTriggered && !this.autoInit)
    ) {
      return;
    }
  ...
  public applyGestures(): void {
    this.currentSlide.slide.layout.on(
      "tap",
    ...
    this.currentSlide.slide.layout.on(
      "pinch",
      (args: gestures.PinchGestureEventData): void => {
        if (this.zoomEnabled) {
          this.onPinch(args);
        }
      }
    ...
    if (args.state === gestures.GestureStateTypes.began) {
      ...
      if (
        ...
        if (this.hasNext) {
          ...
          this.currentSlide.slide.layout.translateX =
        ...
        if (this.hasPrevious) {
          ...
          this.currentSlide.slide.layout.translateX =
            args.deltaX - this.pageWidth;
          this.currentSlide.left.slide.layout.translateX =
  ...
  public GoToSlide(
    num: number,
    traverseDuration: number = 50,
    landingDuration: number = 200
  ): void {
    ...
    else this.previousSlide(duration).then(() => this.GoToSlide(num));
  }
